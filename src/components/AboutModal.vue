...
import { X, Info, Github, ExternalLink } from 'lucide-vue-next';
  ...
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div class="relative bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
      ...
      <div class="p-8 flex flex-col items-center text-center">
        ...
        <h3 class="text-2xl font-bold text-white mb-2">快捷键管理助手</h3>
        <p class="text-slate-400 text-sm mb-6">版本 v1.0.0 (Beta)</p>
        ...
        <div class="flex flex-col w-full gap-3">
          <button 
            ...
            class="flex items-center justify-center gap-2 w-full py-2.5 px-4 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl transition-colors text-sm"
          >
      ...
      <div class="px-6 py-4 bg-slate-800/30 border-t border-slate-800 text-center">
        <p class="text-xs text-slate-500">&copy; 2026 Jeffrey. All rights reserved.</p>
      </div>
    </div>
  </div>
</template>
