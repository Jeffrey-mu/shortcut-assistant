...
<template>
  ...
  >
    <div
      ...
      :class="compact ? 'bg-transparent' : 'bg-slate-950/55 backdrop-blur-md dark:bg-black/76'"
      @click="emit('close')"
    ></div>
    
    <div
      class="modal-shell relative flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 transition-colors duration-300"
      :class="compact ? 'max-h-[calc(100vh-1rem)] w-[min(92vw,360px)] rounded-xl' : 'max-h-[min(86vh,720px)] w-[min(92vw,520px)] rounded-2xl'"
    >
      <div
        class="tool-header shrink-0 transition-colors duration-300"
      ...
      >
        ...
        <div
          class="segmented-control grid grid-cols-4 gap-1"
      ...
      <div class="app-scrollbar min-h-0 flex-1 overflow-y-auto" :class="compact ? 'p-3' : 'p-5'">
        <div v-if="activeTab === 'general'" class="space-y-3">
          <div class="control-card flex items-center justify-between p-3 transition-colors duration-300">
            <div>
        ...
        <div v-else-if="activeTab === 'window'" class="space-y-3">
          ...
          <div class="control-card p-3 transition-colors duration-300">
      ...
      <div
        class="tool-header flex shrink-0 justify-end gap-3 border-t transition-colors duration-300"
        :class="compact ? 'px-3 py-2.5' : 'px-5 py-4'"
      >
        <button 
          @click="emit('close')"
